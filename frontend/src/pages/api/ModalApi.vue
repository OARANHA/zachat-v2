 ...
 * ModalApi.vue
 * Ajustes 14/12/2025:
 * - Proteção no computed cSessions (Array.isArray).
 * - Normalização do objeto api (data/resetarApi).
 * - Tratamento seguro de apiEdicao em abrirModal.
 */
<template>
  <q-dialog
    persistent
  ...
  >
    ...
    >
      <q-card-section>
        <div class="text-h6">
          {{ apiEdicao.id ? 'Editar' : 'Criar' }} Configuração API
        </div>
      </q-card-section>
...
export default {
  ...
  computed: {
    ...
    cSessions () {
      const list = Array.isArray(this.whatsapps) ? this.whatsapps : []
      return list.filter(w => w.type === 'whatsapp' && !w.isDeleted)
    }
  },
  methods: {
    resetarApi () {
    ...
    abrirModal () {
      if (this.apiEdicao && this.apiEdicao.id) {
